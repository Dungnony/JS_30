...
function render(filteredProducts = null) {
  ...
  const total = list.reduce((sum, p) => sum + p.price * p.quantity, 0);
  totalValueSpan.textContent = formatCurrency(total);

  if (list.length > 0) {
    const maxP = list.reduce((max, p) => p.price > max.price ? p : max, list[0]);
    maxPriceProductSpan.textContent = `${maxP.name} (${formatCurrency(maxP.price)})`;
  } else {
    maxPriceProductSpan.textContent = 'Chưa có';
  }
...
searchBtn.onclick = () => {
  ...
  if (!id) {
    alert('Vui lòng nhập mã sản phẩm hợp lệ để tìm kiếm!');
    return;
  }
